odoo.define('dms_field.DocumentsController', function(require) {
"use strict";
...
var files = require('dms_field.files');
var async = require('dms_field.async');
var mimetype = require('dms_field.mimetype');

var Widget = require('web.Widget');

var FileUpload = require('dms_field.FileUpload');
var DocumentDropFileDialog = require('dms_field.DocumentDropFileDialog');
var DocumentDropFilesDialog = require('dms_field.DocumentDropFilesDialog');
var DocumentFileInfoDialog = require('dms_field.DocumentFileInfoDialog');
var DocumentDirectoryInfoDialog = require('dms_field.DocumentDirectoryInfoDialog');
...
var DocumentsController = Widget.extend(FileUpload, {
    ...
    refresh: function(message) {
    	...
    	if(message && message.ids && message.model && !message.create) {
    		...
    		if(id && message.model) {
    			var field = message.model === 'dms.directory' ?
    				'parent_id' : 'directory';
    			this._rpc({
    				 fields: [field],
    				 domain: [['id', '=', id]],
    ...
    getSelectedDirectory: function() {
    	...
    	var directories = _.filter(selected, function(node) {
    		return node.data.odoo_model === "dms.directory";
    	});
...
//    	$("#menuContinenti").prop('disabled', function (_, val) { return ! val; });
    ...
    _openInfo: function(node) {
		if(node.data.odoo_model == "dms.file") {
    ...
    _createNode: function(node, type) {
    	...
    	} else if(type == "dms.directory") {
    		context = $.extend(session.user_context, {
    			default_parent_id: node.data.odoo_id
            });
    	}
    	this.do_action({
    		type: 'ir.actions.act_window',
    ...
    _moveNode: function(ev) {
		...
		} else if(node.data.odoo_model === "dms.directory") {
			vals.parent_id = parent.data.odoo_id;
		}
		this._rpc({
    ...
    _copyNode: function(ev) {
		...
		if(original.data.odoo_model === "dms.file") {
			vals.directory = parent.data.odoo_id;
		} else if(original.data.odoo_model === "dms.directory") {
			vals.parent_id = parent.data.odoo_id;
		}
		this._rpc({
