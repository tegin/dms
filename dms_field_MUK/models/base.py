...
class Base(models.AbstractModel):
    ...
    def _check_parent_field(self):
        if self._parent_name not in self._fields:
            raise TypeError(
                "The parent (%s) field does not exist." % self._parent_name)
    ...
    def _build_search_childs_domain(self, parent_id, domain=[]):
        ...
        return expression.AND(
            [parent_domain, domain]) if domain else parent_domain
    ...
    @api.model
    def search_parents(self, domain=[], offset=0, limit=None, order=None,
            count=False):
        ...
        """
        res = self._search_parents(domain=domain, offset=offset, limit=limit,
                                   order=order, count=count)
        return res if count else self.browse(res)

    @api.model
    def search_read_parents(self, domain=[], fields=None, offset=0, limit=None,
            order=None):
        """ This method finds the top level elements of the hierarchy for a given search query.
            ...
            :returns: the top level elements for the given search query
        """
        records = self.search_parents(domain=domain, offset=offset,
    ...
    @api.model
    def _search_parents(self, domain=[], offset=0, limit=None, order=None,
            count=False):
        self._check_parent_field()
        ...
        return uniquify_list([x[0] for x in self._cr.fetchall()])

    @api.model
    def search_childs(self, parent_id, domain=[], offset=0, limit=None,
    ...
    @api.model
    def search_read_childs(self, parent_id, domain=[], fields=None, offset=0,
        ...
        """
        domain = self._build_search_childs_domain(parent_id, domain=domain)
        return self.search_read(domain=domain, fields=fields, offset=offset,
                                limit=limit, order=order)
