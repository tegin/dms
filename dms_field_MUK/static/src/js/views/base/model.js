odoo.define('dms_field_muk.DocumentsModel', function(require) {
"use strict";
...
var DocumentsModel = Class.extend(EventDispatcherMixin, ServicesMixin, {
    ...
    _loadFiles: function(operator, value, args) {
    	return this._rpc({
            ...
            fields: _.union(args.file.fields || [], [
            	'permission_read', 'permission_create', 
            	'permission_write', 'permission_unlink',
            	'name', 'res_mimetype', 'directory_id', 'size',
    	...
    	return this._rpc({
            ...
            kwargs: {
                ...
                args: this._buildDomain(
                		[['parent_id', 'child_of', directory_id]],
                		args.directory.domain
                ),
    ...
    _searchFiles: function(directory_id, value, args) {
    	...
    	this._rpc({
            ...
            method: 'search',
            kwargs: {
            	args: [['parent_id', 'child_of', directory_id]],
            	context: args.parent_id.context || session.user_context,
            },
        }).then(function(ids) {
        	this._rpc({
                model: 'dms.file',
            	...
            	var directories = _.map(files, function(file) {
            	    console.log(file)
            		return "directory_" + file.directory_id[0];
            	});
    ...
    _loadNode: function(node, args) {
    	var result = $.Deferred();
    	if(node.data && node.data.odoo_model === "dms.storage") {
    ...
    _makeNodeDirectory: function(directory, showFiles) {
    	var directoryNode = {
			...
			data: {
				...
				parent: directory.parent_id ? "directory_" + directory.parent_id[0] : "#",
    ...
    _makeNodeFile: function(file) {
    	...
    	return {
			...
			data: {
				...
				parent: "directory_" + file.directory_id[0],
